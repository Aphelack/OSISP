...
**Описание:** Приложение демонстрирует процесс, который может перезапускаться с сохранением своего состояния (счетчик и текстовое сообщение). При закрытии процесс сохраняет данные в разделяемую память, создает дочерний процесс и завершается. Новый процесс загружает сохраненное состояние и продолжает работу.

**Расположение:** `task1_self_restoring/`

**Запуск:**
```bash
cd task1_self_restoring
./run.sh
# или
make run
```

**Подробности:** См. [task1_self_restoring/README.md](task1_self_restoring/README.md)
